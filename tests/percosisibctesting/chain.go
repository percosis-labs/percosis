...
func SetupTestingApp() (ibctesting.TestingApp, map[string]json.RawMessage) {
	percosisApp := app.Setup(false)
	return percosisApp, app.NewDefaultGenesisState()
}
...
func (chain *TestChain) MoveEpochsToTheFuture() error {
	epochsKeeper := chain.GetPercosisApp().EpochsKeeper
...
// GetPercosisApp returns the current chain's app as an PercosisApp
func (chain *TestChain) GetPercosisApp() *app.PercosisApp {
	v, _ := chain.App.(*app.PercosisApp)
	return v
}
