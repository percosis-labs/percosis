...
depending on the Percosis codebase. It initializes the chains for testing
via Docker files. As a result, the test suite may provide the desired
Percosis version to Docker containers during the initialization. This
design allows for the opportunity of testing chain upgrades in the
future by providing an older Percosis version to the container,
...
1. Chain Initialization
    ...
    of the previous Percosis version, inside `configurer/upgrade.go`. This is
    ...
    `UpgradeConfigurer` takes older percosis versions and configures chains from them. 
    ...
    current branch version of Percosis codebase
    ...
    with IBC initialized at the previous Percosis version

    - If none are true, we only need one chain at the current branch version
    of the Percosis code
...
2. Setting up e2e components
    ...
    - IBC testing
        ...
        - Upgrade Testing
        - 2 chains of the older Percosis version are created, and
    ...
    - Upgrade testing
        ...
        - Current branch Percosis version is spun up to continue with testing
    ...
    [Example](https://github.com/percosis-labs/percosis/blob/c5d5c9f0c6b5c7fdf9688057eb78ec793f6dd580/tests/e2e/configurer/configurer.go#L166)
...
This package directly depends on the Percosis codebase.
...
production percosis container to start the `percosisd` service.
...
### To build the debug Percosis image
...
- `PERCOSIS_E2E_SKIP_UPGRADE` - when true, skips the upgrade tests.
If PERCOSIS_E2E_SKIP_IBC is true, this must also be set to true because upgrade
tests require IBC logic.

- `PERCOSIS_E2E_SKIP_IBC` - when true, skips the IBC tests.

- `PERCOSIS_E2E_SKIP_STATE_SYNC` - when true, skips the state sync tests.

- `PERCOSIS_E2E_SKIP_CLEANUP` - when true, avoids cleaning up the e2e Docker
containers.

- `PERCOSIS_E2E_FORK_HEIGHT` - when the above "IS_FORK" env variable is set to true, this is the string
of the height in which the network should fork. This should match the ForkHeight set in constants.go

- `PERCOSIS_E2E_UPGRADE_VERSION` - string of what version will be upgraded to (for example, "v10")

- `PERCOSIS_E2E_DEBUG_LOG` - when true, prints debug logs from executing CLI commands
    ...
    "configurations": [
        {
            ...
            "env": {
                "PERCOSIS_E2E": "True",
                "PERCOSIS_E2E_SKIP_IBC": "true",
                "PERCOSIS_E2E_SKIP_UPGRADE": "true",
                "PERCOSIS_E2E_SKIP_CLEANUP": "true",
                "PERCOSIS_E2E_SKIP_STATE_SYNC": "true",
                "PERCOSIS_E2E_UPGRADE_VERSION": "v13",
                "PERCOSIS_E2E_DEBUG_LOG": "true",
            },
...
1. Order of executions of tests in `e2e_test.go` is alphabetic.
    ...
    A way to deal with this problem: disable upgrade logic by setting `PERCOSIS_E2E_SKIP_UPGRADE` to `false` and see how test performs.
...
We set the following parameters Hermes configs to enable the consensus min fee in Percosis:
    ...
    In Percosis, we set consensus min fee = .0025 ufury / gas * 400000 gas = 1000
...
This section contains information about debugging percosis's `e2e` tests.
...
1. Executing commands on docker containers
    ...
    Example: `docker exec perco-test-a-node-prune-nothing-snapshot percosisd status` will print a node status of `perco-test-a-node-prune-nothing-snapshot` container. 
...
2. Viewing docker container logs
    ...
    Example: `docker logs perco-test-a-node-prune-nothing-snapshot` will print logs emitted by container `perco-test-a-node-prune-nothing-snapshot` to your standard output.

3. Viewing docker container logs when run percosis's `e2e` tests in CI
