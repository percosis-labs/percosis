...
func (s *IntegrationTestSuite) ProtoRev() {
	...
	// The module should have only percosis as a supported base denom by default.
	supportedBaseDenoms, err := chainANode.QueryProtoRevBaseDenoms()
	s.T().Logf("checking that the protorev module has only percosis as a supported base denom on init: %v", supportedBaseDenoms)
	s.Require().NoError(err)
...
func (s *IntegrationTestSuite) ConcentratedLiquidity() {
	...
	// Calculate the amount of perco required to:
...
func (s *IntegrationTestSuite) IBCTokenTransferAndCreatePool() {
	...
	chainANode.SendIBC(chainB, chainB.NodeConfigs[1].PublicAddress, initialization.PercoToken)
	chainBNode.SendIBC(chainA, chainA.NodeConfigs[1].PublicAddress, initialization.PercoToken)
...
func (s *IntegrationTestSuite) IBCTokenTransferRateLimiting() {
	...
	percoSupply, err := chainANode.QuerySupplyOf("ufury")
	s.Require().NoError(err)

	f, err := percoSupply.ToDec().Float64()
	s.Require().NoError(err)

	over := f * 0.02

	paths := fmt.Sprintf(`{"channel_id": "channel-0", "denom": "%s", "quotas": [{"name":"testQuota", "duration": 86400, "send_recv": [1, 1]}] }`, initialization.PercoToken.Denom)

	// Sending >1%
	fmt.Println("Sending >1%")
	chainANode.SendIBC(chainB, chainB.NodeConfigs[0].PublicAddress, sdk.NewInt64Coin(initialization.PercoDenom, int64(over)))
	...
	s.Eventually(
		...
		1*time.Minute,
		10*time.Millisecond,
		"Percosis node failed to retrieve params",
	)

	// Sending <1%. Should work
	fmt.Println("Sending <1%. Should work")
	chainANode.SendIBC(chainB, chainB.NodeConfigs[0].PublicAddress, sdk.NewInt64Coin(initialization.PercoDenom, 1))
...
func (s *IntegrationTestSuite) IBCWasmHooks() {
	...
	// sender wasm addr
	senderBech32, err := ibchookskeeper.DeriveIntermediateSender("channel-0", validatorAddr, "perco")
...
func (s *IntegrationTestSuite) ArithmeticTWAP() {
	s.T().Skip("TODO: investigate further: https://github.com/percosis-labs/percosis/issues/4342")
...
func (s *IntegrationTestSuite) StateSync() {
	...
	tempDir, err := os.MkdirTemp("", "percosis-e2e-statesync-")
	s.Require().NoError(err)
...
// Tests that v16 upgrade correctly creates the canonical PERCO-DAI pool in the upgrade.
...
func (s *IntegrationTestSuite) ConcentratedLiquidity_CanonicalPool_And_Parameters() {
	...
	// Taken from: https://app.percosis.zone/pool/674
	concentratedPoolId := chainANode.QueryConcentratedPooIdLinkFromCFMM(config.DaiPercoPoolIdv16)
	...
	balancerDaiPercoPool := s.updatedCFMMPool(chainANode, config.DaiPercoPoolIdv16)
	expectedSpotPrice, err := balancerDaiPercoPool.SpotPrice(sdk.Context{}, v16.DAIIBCDenom, v16.DesiredDenom0)
	s.Require().NoError(err)
