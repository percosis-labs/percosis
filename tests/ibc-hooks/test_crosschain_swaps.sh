...
# setup the keys
echo "bottom loan skill merry east cradle onion journey palm apology verb edit desert impose absurd oil bubble sweet glove shallow size build burst effort" | percosisd --keyring-backend test keys add validator --recover || echo "key exists"
echo "increase bread alpha rigid glide amused approve oblige print asset idea enact lawn proof unfold jeans rabbit audit return chuckle valve rather cactus great" | percosisd --keyring-backend test keys add faucet --recover || echo "key exists"

export VALIDATOR=$(percosisd keys show validator -a)
...
MSG=$(jenv -c '{"swap_contract": $SWAPROUTER_ADDRESS, "channels": [["perco", $CHANNEL_ID]]}')
...
MEMO=$(jenv -c '{"wasm": {"contract": $CROSSCHAIN_SWAPS_ADDRESS, "msg": {"percosis_swap":{"swap_amount":"100","output_denom":"ufury","slippage":{"twap": {"slippage_percentage":"20", "window_seconds": 10}},"receiver":$VALIDATOR, "on_failed_delivery": "do_nothing"}}}}')
