...
type HooksTestSuite struct {
	...
	chainC *percosisibctesting.TestChain
...
func (suite *HooksTestSuite) SetupTest() {
	...
	suite.chainC = &percosisibctesting.TestChain{
...
func (suite *HooksTestSuite) GetChain(name Chain) *percosisibctesting.TestChain {
	switch name {
	case ChainA:
		return suite.chainA
	case ChainB:
...
func (suite *HooksTestSuite) TestOnRecvPacketHooks() {
	...
	testCases := []struct {
		...
		{"override", func(status *testutils.Status) {
			suite.chainB.GetPercosisApp().TransferStack.
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.msg, func() {
			...
			ack := suite.chainB.GetPercosisApp().TransferStack.
			...
			if _, ok := suite.chainB.GetPercosisApp().TransferStack.
...
func (suite *HooksTestSuite) receivePacketWithSequence(receiver, memo string, prevSequence uint64) []byte {
	...
	err := suite.chainB.GetPercosisApp().HooksICS4Wrapper.SendPacket(
...
func (suite *HooksTestSuite) TestFundsAreTransferredToTheContract() {
	...
	balance := suite.chainA.GetPercosisApp().BankKeeper.GetBalance(suite.chainA.GetContext(), addr, localDenom)
...
func (suite *HooksTestSuite) TestPacketsThatShouldBeSkipped() {
	...
	testCases := []struct {
		...
		{`{"wasm": {"contract": "perco1clpqr4nrk4khgkxj78fcwwh6dl3uw4epasmvnj"}}`, false},
		{`{"wasm": {"msg": "something"}}`, false},
		// invalid receiver
		{`{"wasm": {"contract": "perco1clpqr4nrk4khgkxj78fcwwh6dl3uw4epasmvnj", "msg": {}}}`, false},
		// msg not an object
...
func (suite *HooksTestSuite) TestFundTracking() {
	...
	senderLocalAcc, err := ibchookskeeper.DeriveIntermediateSender("channel-0", suite.chainB.SenderAccount.GetAddress().String(), "perco")
	suite.Require().NoError(err)
...
func (suite *HooksTestSuite) FullSend(msg sdk.Msg, direction Direction) (*sdk.Result, *sdk.Result, string, error) {
	var sender *percosisibctesting.TestChain
	switch direction {
	case AtoB:
		sender = suite.chainA
	case BtoA:
...
func (suite *HooksTestSuite) SetupPools(chainName Chain, multipliers []sdk.Dec) []gammtypes.CFMMPoolI {
	...
	bondDenom := chain.GetPercosisApp().StakingKeeper.BondDenom(chain.GetContext())
	...
	for index, multiplier := range multipliers {
		...
		poolId, err := chain.GetPercosisApp().PoolManagerKeeper.CreatePool(chain.GetContext(), msg)
		suite.Require().NoError(err)

		pool, err := chain.GetPercosisApp().GAMMKeeper.GetPoolAndPoke(chain.GetContext(), poolId)
		suite.Require().NoError(err)
...
func (suite *HooksTestSuite) SetupCrosschainSwaps(chainName Chain) (sdk.AccAddress, sdk.AccAddress) {
	...
	bankKeeper := chain.GetPercosisApp().BankKeeper
	...
	msg := `{  
		"modify_bech32_prefixes": {
		  "operations": [
			{"operation": "set", "chain_name": "percosis", "prefix": "perco"},
			{"operation": "set", "chain_name": "chainB", "prefix": "perco"},
			{"operation": "set", "chain_name": "chainB-cw20", "prefix": "perco"},
			{"operation": "set", "chain_name": "chainC", "prefix": "perco"}
		  ]
		}
	  }
	  `
...
func (suite *HooksTestSuite) fundAccount(chain *percosisibctesting.TestChain, owner sdk.AccAddress) {
...
func (suite *HooksTestSuite) SetupCrosschainRegistry(chainName Chain) (sdk.AccAddress, string, string, string) {
	...
	// Fund the account with some ufury and some stake.
	for _, ch := range []*percosisibctesting.TestChain{suite.chainA, suite.chainB, suite.chainC} {
	...
	_, err := sdk.Bech32ifyAddressBytes("perco", registryAddr)
	suite.Require().NoError(err)
...
func (suite *HooksTestSuite) setChainChannelLinks(registryAddr sdk.AccAddress, chainName Chain) {
	...
	msg := `{
		"modify_chain_channel_links": {
		  "operations": [
			{"operation": "set","source_chain": "chainB","destination_chain": "percosis","channel_id": "channel-0"},
			{"operation": "set","source_chain": "percosis","destination_chain": "chainB","channel_id": "channel-0"},
			...
			{"operation": "set","source_chain": "percosis","destination_chain": "chainC","channel_id": "channel-1"},
			{"operation": "set","source_chain": "chainC","destination_chain": "percosis","channel_id": "channel-1"},
			{"operation": "set","source_chain": "percosis","destination_chain": "chainB-cw20","channel_id": "channel-2"},
			{"operation": "set","source_chain": "chainB-cw20","destination_chain": "percosis","channel_id": "channel-2"}
		  ]
		}
	  }
	  `
	...
	msg := `{
		"modify_chain_channel_links": {
		  "operations": [
			...
			{"operation": "change","source_chain": "chainB","destination_chain": "percosis","new_source_chain": "chainD"},
			{"operation": "change","source_chain": "percosis","destination_chain": "chainB","new_destination_chain": "chainD"}
		  ]
		}
	  }
	  `
...
func (suite *HooksTestSuite) TestCrosschainRegistry() {
	...
	contractAlias := "percosis_registry_contract"
	...
	channelQuery := `{"get_channel_from_chain_pair": {"source_chain": "percosis", "destination_chain": "chainB"}}`
	...
	_, err := suite.chainA.GetPercosisApp().WasmKeeper.QuerySmart(suite.chainA.GetContext(), registryAddr, []byte(channelQuery))
	suite.Require().Error(err)
	suite.Require().Contains(err.Error(), "RegistryValue not found")

	// Unwrap token0CB and check that the path has changed
	channelQuery = `{"get_channel_from_chain_pair": {"source_chain": "percosis", "destination_chain": "chainD"}}`
	channelQueryResponse = suite.chainA.QueryContractJson(&suite.Suite, registryAddr, []byte(channelQuery))
	suite.Require().Equal("channel-0", channelQueryResponse.Str)
}
...
func (suite *HooksTestSuite) TestUnwrapToken() {
	...
	msg := `{
		"modify_bech32_prefixes": {
		  "operations": [
			{"operation": "set", "chain_name": "percosis", "prefix": "perco"},
			{"operation": "set", "chain_name": "chainA", "prefix": "perco"},
			{"operation": "set", "chain_name": "chainB", "prefix": "perco"},
			{"operation": "set", "chain_name": "chainC", "prefix": "perco"}
		  ]
		}
	...
	testCases := []struct {
		...
		{ChainA, "percosis", token0CBA, token0CA, []Direction{AtoB, BtoC, CtoA}},
	...
	for _, tc := range testCases {
		...
		receiverApp := receiverChain.GetPercosisApp()
		initialSenderBalance := percosisApp.BankKeeper.GetBalance(suite.chainA.GetContext(), owner, tc.sentToken)
		...
		finalSenderBalance := percosisApp.BankKeeper.GetBalance(suite.chainA.GetContext(), owner, tc.sentToken)
...
func (suite *HooksTestSuite) TestCrosschainSwaps() {
	...
	balanceSender := percosisApp.BankKeeper.GetBalance(suite.chainA.GetContext(), owner, "token0")

	ctx := suite.chainA.GetContext()

	msg := fmt.Sprintf(`{"percosis_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s", "on_failed_delivery": "do_nothing"}}`,
	...
	balanceSender2 := percosisApp.BankKeeper.GetBalance(suite.chainA.GetContext(), owner, "token0")
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCTest() {
	...
	percosisAppB := suite.chainB.GetPercosisApp()
	balanceToken0 := percosisAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), initializer, token0IBC)
	receiver := initializer
	balanceToken1 := percosisAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), receiver, token1IBC)

	suite.Require().Equal(int64(0), balanceToken1.Amount.Int64())

	// Generate swap instructions for the contract
	swapMsg := fmt.Sprintf(`{"percosis_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s", "on_failed_delivery": "do_nothing", "next_memo":{}}}`,
		receiver,
	)
	// Generate full memo
	...
	balanceToken0After := percosisAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), initializer, token0IBC)
	suite.Require().Equal(int64(1000), balanceToken0.Amount.Sub(balanceToken0After.Amount).Int64())

	balanceToken1After := percosisAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), receiver, token1IBC)
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCBadAck() {
	...
	swapMsg := fmt.Sprintf(`{"percosis_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s","on_failed_delivery": {"local_recovery_addr": "%s"}, "next_memo": %s }}`,
		receiver,
		recoverAddr,
	...
	percosisAppA := suite.chainA.GetPercosisApp()
	balanceContract := percosisAppA.BankKeeper.GetBalance(suite.chainA.GetContext(), crosschainAddr, "token1")
	...
	swapMsg2 := fmt.Sprintf(`{"percosis_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s","on_failed_delivery": {"local_recovery_addr": "%s"}, "next_memo": %s }}`,
		receiver,
		recoverAddr2,
		`{"wasm": "bad wasm specifier"}`,
	)
	...
	balanceContract2 := percosisAppA.BankKeeper.GetBalance(suite.chainA.GetContext(), crosschainAddr, "token1")
	...
	balanceRecovery := percosisAppA.BankKeeper.GetBalance(suite.chainA.GetContext(), recoverAddr2, "token1")
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCBadSwap() {
	...
	swapMsg := fmt.Sprintf(`{"percosis_swap":{"output_denom":"token1","slippage":{"min_output_amount":"50000"},"receiver":"chainB/%s", "on_failed_delivery": "do_nothing"}}`,
		receiver,
	)
	// Generate full memo
	msg := fmt.Sprintf(`{"wasm": {"contract": "%s", "msg": %s } }`, crosschainAddr, swapMsg)
...
func (suite *HooksTestSuite) TestBadCrosschainSwapsNextMemoMessages() {
	...
	innerMsg := fmt.Sprintf(`{"percosis_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s","on_failed_delivery": {"local_recovery_addr": "%s"},"next_memo":%%s}}`,
...
func (suite *HooksTestSuite) CreateIBCPoolOnChain(chainName Chain, denom1, denom2 string, amount1 sdk.Int) uint64 {
	...
	_, err = chain.GetPercosisApp().GAMMKeeper.GetPoolAndPoke(chain.GetContext(), poolId)
	suite.Require().NoError(err)
	return poolId
}
...
func (suite *HooksTestSuite) TestCrosschainForwardWithMemo() {
	...
	balanceToken0IBCBefore := suite.chainA.GetPercosisApp().BankKeeper.GetBalance(suite.chainA.GetContext(), receiver, token0IBC)
	...
	// Generate swap instructions for the contract
	//
	// Note: Both chains think of themselves as "percosis" and the other as "chainB". That is, the registry
	...
	// be relevant here
	nextMemo := fmt.Sprintf(`{"wasm": {"contract": "%s", "msg": {"percosis_swap":{"output_denom":"token0","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"ibc:channel-0/%s", "on_failed_delivery": "do_nothing"}}}}`,
		crosschainAddrB,
		receiver,
	)
	swapMsg := fmt.Sprintf(`{"percosis_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"ibc:channel-0/%s", "on_failed_delivery": "do_nothing", "next_memo": %s}}`,
		crosschainAddrB,
		nextMemo,
	)
	fmt.Println(swapMsg)
	...
	balanceToken0IBCAfter := suite.chainA.GetPercosisApp().BankKeeper.GetBalance(suite.chainA.GetContext(), receiver, token0IBC)
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCMultiHop() {
	...
	percosisAppB := suite.chainB.GetPercosisApp()
	balanceToken0 := percosisAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), accountB, token0ACB)
	balanceToken1 := percosisAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), accountB, token1AB)
	...
	balanceToken0After := percosisAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), accountB, token0ACB)
	suite.Require().Equal(int64(1000), balanceToken0.Amount.Sub(balanceToken0After.Amount).Int64())

	balanceToken1After := percosisAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), accountB, token1AB)
	suite.Require().Greater(balanceToken1After.Amount.Int64(), int64(0))
}
...
func (suite *HooksTestSuite) TestMultiHopXCS() {
	...
	testCases := []struct {
		...
		{
			name: "Native to PercoNative into same chain",
		...
		{
			name:          "PercoNative to Native into same chain",
			sender:        actorChainB,
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			sentTokenBalance := senderChain.GetPercosisApp().BankKeeper.GetBalance(senderChain.GetContext(), tc.sender.address, initialToken)
			fmt.Println("sentTokenBalance", sentTokenBalance)
			suite.Require().True(sentTokenBalance.Amount.GTE(sendAmount))
			// get receiver balance
			receivedTokenBalance := receiverChain.GetPercosisApp().BankKeeper.GetBalance(receiverChain.GetContext(), tc.receiver.address, tc.receivedToken)

			// Generate swap instructions for the contract
			swapMsg := fmt.Sprintf(`{"percosis_swap":{"output_denom":"%s","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"%s/%s", "on_failed_delivery": "do_nothing", "next_memo":{}}}`,
			...
			sentTokenAfter := senderChain.GetPercosisApp().BankKeeper.GetBalance(senderChain.GetContext(), tc.sender.address, initialToken)
			suite.Require().Equal(sendAmount.Int64(), sentTokenBalance.Amount.Sub(sentTokenAfter.Amount).Int64())

			fmt.Println(receiverChain.GetPercosisApp().BankKeeper.GetAllBalances(receiverChain.GetContext(), tc.receiver.address))

			receivedTokenAfter := receiverChain.GetPercosisApp().BankKeeper.GetBalance(receiverChain.GetContext(), tc.receiver.address, tc.receivedToken)
...
func (suite *HooksTestSuite) ExecuteOutpostSwap(initializer, receiverAddr sdk.AccAddress, receiver string) {
	...
	outpostAddr := suite.chainB.InstantiateContract(&suite.Suite,
		fmt.Sprintf(`{"crosschain_swaps_contract": "%s", "percosis_channel": "channel-0"}`, crosschainAddr), 1)
	...
	percosisAppB := suite.chainB.GetPercosisApp()
	balanceToken0 := percosisAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), initializer, token0IBC)
	balanceToken1 := percosisAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), receiverAddr, token1IBC)

	suite.Require().Equal(int64(0), balanceToken1.Amount.Int64())

	// Generate swap instructions for the contract
	swapMsg := fmt.Sprintf(`{"percosis_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"%s", "on_failed_delivery": "do_nothing"}}`,
		receiver,
	)

	// Call the outpost
	contractKeeper := wasmkeeper.NewDefaultPermissionKeeper(percosisAppB.WasmKeeper)
	...
	balanceToken1After := percosisAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), receiverAddr, token1IBC)
