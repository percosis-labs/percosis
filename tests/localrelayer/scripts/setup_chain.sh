...
add_genesis_accounts () {
    ...
    echo $VALIDATOR_MNEMONIC | percosisd keys add $VALIDATOR_MONIKER --recover --keyring-backend=test --home $PERCOSIS_HOME
    VALIDATOR_ACCOUNT=$(percosisd keys show -a $VALIDATOR_MONIKER --keyring-backend test --home $PERCOSIS_HOME)
    percosisd add-genesis-account $VALIDATOR_ACCOUNT 100000000000ufury,100000000000uion,100000000000stake --home $PERCOSIS_HOME
    
    # Faucet
    echo "🚰 Add faucet account"
    echo $FAUCET_MNEMONIC | percosisd keys add faucet --recover --keyring-backend=test --home $PERCOSIS_HOME
    FAUCET_ACCOUNT=$(percosisd keys show -a faucet --keyring-backend test --home $PERCOSIS_HOME)
    percosisd add-genesis-account $FAUCET_ACCOUNT 100000000000ufury,100000000000uion,100000000000stake --home $PERCOSIS_HOME

    # Relayer
    echo "🔗 Add relayer account"
    echo $RELAYER_MNEMONIC | percosisd keys add relayer --recover --keyring-backend=test --home $PERCOSIS_HOME
    RELAYER_ACCOUNT=$(percosisd keys show -a relayer --keyring-backend test --home $PERCOSIS_HOME)
    percosisd add-genesis-account $RELAYER_ACCOUNT 1000000000ufury,1000000000uion,1000000000stake --home $PERCOSIS_HOME
    
    percosisd gentx $VALIDATOR_MONIKER 500000000ufury --keyring-backend=test --chain-id=$CHAIN_ID --home $PERCOSIS_HOME
...
then
    install_prerequisites
    echo "🧪 Creating Percosis home for $VALIDATOR_MONIKER"
    echo $VALIDATOR_MNEMONIC | percosisd init -o --chain-id=$CHAIN_ID --home $PERCOSIS_HOME --recover $VALIDATOR_MONIKER
    edit_genesis
    add_genesis_accounts
    edit_config
fi
