...
PERCOSIS_HOME=$HOME/.percosisd-local/
...
make_proposal() {
    ...
    PERCOSIS_CMD="percosisd tx gov submit-proposal software-upgrade \
        $UPGRADE_VERSION \
    ...
    PROPOSAL_JSON=$(eval "$PERCOSIS_CMD")
...
# Query proposal
query_proposal() {
    percosisd q gov proposal $PROPOSAL_ID \
        --node $RPC_NODE \
        -o json | jq
}
...
vote_on_proposal() {
    ...
    PERCOSIS_CMD="percosisd tx gov vote $PROPOSAL_ID yes \
        --from $KEY \
        --chain-id $CHAIN_ID \
        --fees $TX_FEES \
        --node $RPC_NODE \
