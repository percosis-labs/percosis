...
percosisd tx gov submit-proposal wasm-store $CONTRACT.wasm --title "Add $CONTRACT" \
...
percosisd query gov proposal $PROPOSAL

percosisd tx gov deposit $PROPOSAL 10000000stake --from validator --keyring-backend test \
    --chain-id $CHAIN_ID -y -b block --gas 5000000 --gas-prices 0.025stake

percosisd tx gov vote $PROPOSAL yes --from validator --keyring-backend test \
...
percosisd query wasm list-code
