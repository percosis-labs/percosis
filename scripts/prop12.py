...
# 65838 is the epoch detailed in Percosis prop 12
...
perco_issued = (300000000*1000000*0.45)/365

def dict_perco_rewards_by_gauge_id(data):
  ...
  return {gid : (perco_issued*gauge_weights[gid]/total_weight) for gid in gauge_weights}
...
def dict_rewards_by_address(data):
  perco_per_gauge = dict_perco_rewards_by_gauge_id(data)
  ...
  mint_by_gauge_addr = {gid : {addr : perco_per_gauge.get(gid, 0)*locked[gid][addr]/gauge_totals[gid] for addr in locked[gid]} for gid in locked}
  ...
  #convert {gauge_id -> {addr -> ufury_issued}} into {addr -> ufury_issued} by merging the dictionaries for each gauge id, adding the perco issued for each address
...
def compute_sanity_check_and_export(data):
  ...
  community_paid = dict_perco_rewards_by_gauge_id(data)["0"]

  print("expected total: ", int(perco_issued), "ufury")
  print("paid to LPers: ", int(total_paid), "ufury")
  print("paid to community pool: ", int(community_paid), "ufury")
  imbalance = int(perco_issued - (total_paid + community_paid))
