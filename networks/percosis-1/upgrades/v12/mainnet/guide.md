...
Percosis v12 Gov Prop: <https://www.mintscan.io/percosis/proposals/335>

Countdown: <https://www.mintscan.io/percosis/blocks/6246000>
...
cpercovisor/genesis folder and v11 folder.

```{.sh}
cp $GOPATH/bin/percosisd ~/.percosisd/cpercovisor/genesis/bin
mkdir -p ~/.percosisd/cpercovisor/upgrades/v11/bin
cp $GOPATH/bin/percosisd ~/.percosisd/cpercovisor/upgrades/v11/bin
```

Cpercovisor is now ready to be set up for v12.
...
make build
cp build/percosisd ~/.percosisd/cpercovisor/upgrades/v12/bin
```

Now, at the upgrade height, Cpercovisor will upgrade to the v12 binary

## Manual Option

1. Wait for Percosis to reach the upgrade height (FILL ME IN WITH UPGRADE HEIGHT)
