...
transfer on chain A, receiving it on percosis, swapping for a different token,
...
ibc denoms of tokens on percosis, and when to use packet forward middleware vs callbacks.
...
This documentation is for the latest version of the contract. For v1, see the [previous docs](https://github.com/percosis-labs/percosis/tree/v15.x/cosmwasm/contracts/crosschain-swaps)
...
``` json
{"swap_contract": "perco1thiscontract", "governor": "perco1..."}
```
...
Assuming the current implementation of the wasm middleware on Percosis v14 (`x/ibc-hooks/v0.0.6`), the memo
of an IBC transfer to do crosschain swaps would look as follows:

``` json
{"wasm": {
    "contract": "perco1crosschainswapscontract", 
    "msg": {
        "percosis_swap": {
            "output_denom":"token1",
...
via `{"local_recovery_addr": "perco1..."}`. If set to `do_nothing`, the contract will
...
{
  "wasm": {
    "contract": "perco1uwk8xc6q0s6t5qcpr6rht3sczu6du83xq8pwxjua0hfj5hzcnh3sqxwvxs",  // XCS contract
    "msg": {
      "percosis_swap": {
        "output_denom": "ibc/27394FB092D2ECCD56123C74F36E4C1F926001CEADA9CA97EA622B25F41E5EB2",  // ATOM on percosis
        "slippage": {
          "twap": {
...
```bash
junod  --node https://rpc.cosmos.directory:443/juno tx ibc-transfer transfer transfer channel-0 perco1uwk8xc6q0s6t5qcpr6rht3sczu6du83xq8pwxjua0hfj5hzcnh3sqxwvxs 100ujuno   \
--from testaccount -y --gas auto --gas-prices 0.1ujuno --gas-adjustment 1.3 \
--memo '{"wasm":{"contract":"perco1uwk8xc6q0s6t5qcpr6rht3sczu6du83xq8pwxjua0hfj5hzcnh3sqxwvxs","msg":{"percosis_swap":{"output_denom":"ibc/27394FB092D2ECCD56123C74F36E4C1F926001CEADA9CA97EA622B25F41E5EB2","slippage":{"twap":{"slippage_percentage":"20","window_seconds":10}},"receiver":"cosmos1tfu4j7nzfhtex2wyp946rm02748zxu8w0kvt87","on_failed_delivery":"do_nothing"}}}}'
```

TODO: Add an example in TS

#### Native to Percosis native

Similarly, if you wanted to swap for a token that is native to percosis (perco, ion, or tokenfactory tokens) you can use a 
memo like the one above but with the appropriate valies for the token and receiver.

```json
{
  "wasm": {
    "contract": "perco1uwk8xc6q0s6t5qcpr6rht3sczu6du83xq8pwxjua0hfj5hzcnh3sqxwvxs",
    "msg": {
      "percosis_swap": {
        "output_denom": "ufury",  // Percosis native token
        "slippage": {
          "twap": {
            "slippage_percentage": "20",
            "window_seconds": 10
...
```bash
junod  --node https://rpc.cosmos.directory:443/juno tx ibc-transfer transfer transfer channel-0 perco1uwk8xc6q0s6t5qcpr6rht3sczu6du83xq8pwxjua0hfj5hzcnh3sqxwvxs 100738ujuno   \
--from testaccount -y --gas auto --gas-prices 0.1ujuno --gas-adjustment 1.3 \
--memo '{"wasm":{"contract":"perco1uwk8xc6q0s6t5qcpr6rht3sczu6du83xq8pwxjua0hfj5hzcnh3sqxwvxs","msg":{"percosis_swap":{"output_denom":"ufury","slippage":{"twap":{"slippage_percentage":"20","window_seconds":10}},"receiver":"juno1tfu4j7nzfhtex2wyp946rm02748zxu8wey0sqz","on_failed_delivery":"do_nothing"}}}}'
```
...
Sometimes, the token you want to receive is neither native to the chain initiating the swap, nor to percosis.
...
For this example, we will swap PERCO stored in juno to ATOM and send it back to juno. 

If we did this like the previous examples we would end up with a token that isn't recognized 
as ATOM on juno. This is because the pools on percosis use ATOM sent directly from gaia
and not via juno (i.e.: percosis(ATOM) != percosis(juno(ATOM))).
...
Note that in this example we are using PERCO stored in juno, so the token we send is 
`ibc/ED07A3391A112B175915CD8FAF43A2DA8E4790EDE12566649D0C2F97716B8518` and not `ujuno` like in previous examples.

```bash
junod  --node https://rpc.cosmos.directory:443/juno tx ibc-transfer transfer transfer channel-0 perco1uwk8xc6q0s6t5qcpr6rht3sczu6du83xq8pwxjua0hfj5hzcnh3sqxwvxs 100ibc/ED07A3391A112B175915CD8FAF43A2DA8E4790EDE12566649D0C2F97716B8518   \
--from testaccount -y --gas auto --gas-prices 0.1ujuno --gas-adjustment 1.3 \
--memo '{"wasm":{"contract":"perco1uwk8xc6q0s6t5qcpr6rht3sczu6du83xq8pwxjua0hfj5hzcnh3sqxwvxs","msg":{"percosis_swap":{"output_denom":"ibc/27394FB092D2ECCD56123C74F36E4C1F926001CEADA9CA97EA622B25F41E5EB2","receiver":"cosmos1tfu4j7nzfhtex2wyp946rm02748zxu8w0kvt87","slippage":{"twap":{"slippage_percentage":"20","window_seconds":10}},"next_memo":{"forward":{"receiver":"juno1tfu4j7nzfhtex2wyp946rm02748zxu8wey0sqz","port":"transfer","channel":"channel-207"}},"on_failed_delivery":"do_nothing"}}}}'
```
...
to percosis, then we need to do the denom path unwinding as part of the IBC packet sent. 
...
{
    "forward": {
        "receiver": "perco1uwk8xc6q0s6t5qcpr6rht3sczu6du83xq8pwxjua0hfj5hzcnh3sqxwvxs",  // XCS contract
        "port": "transfer",
        "channel": "channel-141",  // Percosis channel on gaia
        "next": {
            "wasm": {
                "contract": "perco1uwk8xc6q0s6t5qcpr6rht3sczu6du83xq8pwxjua0hfj5hzcnh3sqxwvxs",  // XCS contract
                "msg": {
                    "percosis_swap": {
                        "output_denom": "ibc/46B44899322F3CD854D2D46DEEF881958467CDD4B3B10086DA49296BBED94BED",  // Juno denom on percosis
...
--from testaccount -y --gas auto --gas-prices 0.1ujuno --gas-adjustment 1.3 \
--memo '{"forward":{"receiver":"perco1uwk8xc6q0s6t5qcpr6rht3sczu6du83xq8pwxjua0hfj5hzcnh3sqxwvxs","port":"transfer","channel":"channel-141","next":{"wasm":{"contract":"perco1uwk8xc6q0s6t5qcpr6rht3sczu6du83xq8pwxjua0hfj5hzcnh3sqxwvxs","msg":{"percosis_swap":{"output_denom":"ibc/46B44899322F3CD854D2D46DEEF881958467CDD4B3B10086DA49296BBED94BED","receiver":"juno1tfu4j7nzfhtex2wyp946rm02748zxu8wey0sqz","slippage":{"twap":{"slippage_percentage":"20","window_seconds":10}},"next_memo":{"forward":{"receiver":"cosmos1tfu4j7nzfhtex2wyp946rm02748zxu8w0kvt87","port":"transfer","channel":"channel-1"}},"on_failed_delivery":"do_nothing"}}}}}}'
```
...
This guide will walk you through the process of manually testing the cross-chain swapping functionality in Percosis.

To test this contract, we will use the 
[localrelayer tool](https://github.com/percosis-labs/percosis/tree/main/tests/localrelayer). This tool will setup two 
percosis testnets and a relayer between them.


#### Prerequisites
* percosisd command line tool installed and configured.
...
```bash
alias chainA="percosisd --node http://localhost:26657 --chain-id localpercosis-a"
alias chainB="percosisd --node http://localhost:36657 --chain-id localpercosis-b"
```

Prepare other variables that we will use across the test:

```bash
VALIDATOR=$(percosisd keys show validator -a --keyring-backend test)
...
```bash
echo "bottom loan skill merry east cradle onion journey palm apology verb edit desert impose absurd oil bubble sweet glove shallow size build burst effort" | percosisd --keyring-backend test keys add validator --recover
echo "increase bread alpha rigid glide amused approve oblige print asset idea enact lawn proof unfold jeans rabbit audit return chuckle valve rather cactus great" | percosisd --keyring-backend test  keys add faucet --recover
```
...
Chain B will be the chain that represents percosis, so it needs to have IBC'd tokens from chain A so that we can setup 
a pool in it
...
```bash
MSG=$(jenv -c '{"swap_contract": $SWAPROUTER_ADDRESS, "channels": [["perco", $CHANNEL_ID]], "governor": $VALIDATOR}')
...
In this case, if the receiver is "perco", then the tokens will be sent through the channel with id $CHANNEL_ID. 
...
Now the exciting bit! We will perform a crosschain swap from chainA/perco to chainB/ufury.
...
```bash
MEMO=$(jenv -c '{"wasm": {"contract": $CROSSCHAIN_SWAPS_ADDRESS, "msg": {"percosis_swap":{"output_denom":"ufury","slippage":{"twap": {"slippage_percentage":"20", "window_seconds": 10}},"receiver":$VALIDATOR, "on_failed_delivery": "do_nothing"}}}}')
chainA tx ibc-transfer transfer transfer $CHANNEL_ID $CROSSCHAIN_SWAPS_ADDRESS 100ufury \
    --from validator -y "${TX_FLAGS[@]}" \
    --memo "$MEMO"
```

The memo here is a JSON object that describes what percosis should execute when it receives the transaction. 
...
The "percosis_swap" contains the following required fields: output_denom, slippage, receiver, 
...
* An percosis testnet
* Another testnet that supports IBC memo with an open channel to the percosis testnet
* Tokens on the both percosis testnets
