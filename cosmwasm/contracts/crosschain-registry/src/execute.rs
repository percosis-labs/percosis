...
mod tests {
    ...
    fn test_set_contract_alias() {
        ...
        let address = "perco12smx2wdlyttvyzvzg54y2vnqwq2qjatel8rck9".to_string();
        ...
        assert_eq!(
            CONTRACT_ALIAS_MAP
                ...
                .unwrap(),
            "perco12smx2wdlyttvyzvzg54y2vnqwq2qjatel8rck9"
        );
        ...
        let msg = ExecuteMsg::ModifyContractAlias {
            operations: vec![ContractAliasInput {
                operation: Operation::Set,
                alias: alias.clone(),
                address: Some("perco1fsdaf7dsfasndjklk3jndskajnfkdjsfjn3jka".to_string()),
                new_alias: None,
            }],
        };
        ...
        let msg = ExecuteMsg::ModifyContractAlias {
            operations: vec![ContractAliasInput {
                ...
                address: Some("perco1nna7k5lywn99cd63elcfqm6p8c5c4qcuqwwflx".to_string()),
                new_alias: None,
            }],
        };
    ...
    fn test_set_chain_channel_link() {
        ...
        // Set the canonical channel link between percosis and cosmos to channel-0
        ...
        assert_eq!(
            CHAIN_TO_CHAIN_CHANNEL_MAP
                .load(&deps.storage, ("percosis", "cosmos"))
                .unwrap(),
            ("channel-0", true).into()
        );

        // Verify that channel-0 on percosis is linked to cosmos
        assert_eq!(
            CHANNEL_ON_CHAIN_CHAIN_MAP
                .load(&deps.storage, ("channel-0", "percosis"))
                .unwrap(),
            ("cosmos", true).into()
        );

        // Attempt to set the canonical channel link between percosis and cosmos to channel-150
        ...
        // Attempt to set the canonical channel link between mars and percosis to channel-1 with an unauthorized address
        let msg = ExecuteMsg::ModifyChainChannelLinks {
            operations: vec![ConnectionInput {
                operation: FullOperation::Set,
        ...
        let expected_error = ContractError::Unauthorized {};
        assert_eq!(result.unwrap_err(), expected_error);
        assert!(!CHAIN_TO_CHAIN_CHANNEL_MAP.has(&deps.storage, ("mars", "percosis")));

        // Set the canonical channel link between mars and percosis to channel-1 with a mars chain admin address
        let chain_admin_info = mock_info(CHAIN_ADMIN, &[]);
        contract::execute(deps.as_mut(), mock_env(), chain_admin_info.clone(), msg).unwrap();
        assert_eq!(
            CHAIN_TO_CHAIN_CHANNEL_MAP
                .load(&deps.storage, ("mars", "percosis"))
                .unwrap(),
            ("channel-1", true).into()
        );
        assert_eq!(
        ...
        // Note: the chain admin address for mars and perco is the chain maintainer for juno
    ...
    fn test_change_chain_channel_link() {
        ...
        // Change the canonical channel link between percosis and cosmos to channel-150 with the global admin address
        let msg = ExecuteMsg::ModifyChainChannelLinks {
            operations: vec![ConnectionInput {
        ...
        // Verify that the channel between percosis and cosmos has changed from channel-0 to channel-150
        assert_eq!(
            CHAIN_TO_CHAIN_CHANNEL_MAP
                .load(&deps.storage, ("percosis", "cosmos"))
                .unwrap(),
        ...
        // Change channel-0 link of percosis from cosmos to regen with the global admin address
        let msg = ExecuteMsg::ModifyChainChannelLinks {
            operations: vec![ConnectionInput {
                operation: FullOperation::Change,
                source_chain: "percosis".to_string(),
        ...
        // Verify that channel-150 on percosis is linked to regen
        assert_eq!(
            CHANNEL_ON_CHAIN_CHAIN_MAP
                .load(&deps.storage, ("channel-150", "percosis"))
                .unwrap(),
            ("regen", true).into()
        );

        // Attempt to change the canonical channel link between percosis and regen to channel-2 with an unauthorized address
        let msg = ExecuteMsg::ModifyChainChannelLinks {
            operations: vec![ConnectionInput {
                operation: FullOperation::Change,
                source_chain: "percosis".to_string(),
        ...
        // Set the canonical channel link between mars and percosis to channel-1 with a chain admin address
        let info_chain_admin = mock_info(CHAIN_ADMIN, &[]);
        contract::execute(deps.as_mut(), mock_env(), info_chain_admin, msg).unwrap();
        assert_eq!(
            CHAIN_TO_CHAIN_CHANNEL_MAP
                .load(&deps.storage, ("percosis", "regen"))
                .unwrap(),
            ("channel-2", true).into()
        );
        ...
        // Attempt to update a percosis channel link with a percosis chain maintainer address
    ...
    fn test_remove_chain_channel_link() {
        ...
        // Remove the percosis cosmos link with a global admin address
        let msg = ExecuteMsg::ModifyChainChannelLinks {
            operations: vec![ConnectionInput {
                operation: FullOperation::Remove,
                source_chain: "percosis".to_string(),
        ...
        assert!(!CHAIN_TO_CHAIN_CHANNEL_MAP.has(&deps.storage, ("percosis", "cosmos")));
        ...
        // Attempt to remove the percosis regen link with a percosis chain maintainer address
    ...
    fn test_set_bech32_prefix_to_chain() {
        ...
        let msg = ExecuteMsg::ModifyBech32Prefixes {
            operations: vec![ChainToBech32PrefixInput {
                operation: FullOperation::Set,
                chain_name: "PERCOSIS".to_string(),
                prefix: "PERCO".to_string(),
                new_prefix: None,
            }],
        };
        let info = mock_info(CREATOR_ADDRESS, &[]);
        ...
        assert_eq!(
            CHAIN_TO_BECH32_PREFIX_MAP
                .load(&deps.storage, "percosis")
                .unwrap(),
            ("perco", true).into()
        );
        assert_eq!(
            CHAIN_TO_BECH32_PREFIX_REVERSE_MAP
                .load(&deps.storage, "perco")
                .unwrap(),
            vec!["percosis"]
        );
