...
func RunForkLogic(ctx sdk.Context, appKeepers *keepers.AppKeepers) {
	...
	// https://github.com/percosis-labs/percosis/issues/1609
	if ctx.ChainID() != "percosis-1" {
		return
	}
	...
	ctx.Logger().Info("Applying Percosis v8 upgrade. Allowing direct unpooling for whitelisted pools")
