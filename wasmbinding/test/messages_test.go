...
func TestCreateDenom(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// when
			gotErr := wasmbinding.PerformCreateDenom(percosis.TokenFactoryKeeper, percosis.BankKeeper, ctx, actor, spec.createDenom)
			// then
			if spec.expErr {
...
func TestChangeAdmin(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// Setup
			percosis, ctx := SetupCustomApp(t, tokenCreator)

			err := wasmbinding.PerformCreateDenom(percosis.TokenFactoryKeeper, percosis.BankKeeper, ctx, tokenCreator, &bindings.CreateDenom{
				Subdenom: validDenom,
			})
			require.NoError(t, err)

			err = wasmbinding.ChangeAdmin(percosis.TokenFactoryKeeper, ctx, spec.actor, spec.changeAdmin)
...
func TestMint(t *testing.T) {
	...
	err := wasmbinding.PerformCreateDenom(percosis.TokenFactoryKeeper, percosis.BankKeeper, ctx, creator, &validDenom)
	require.NoError(t, err)

	emptyDenom := bindings.CreateDenom{
		Subdenom: "",
	}
	err = wasmbinding.PerformCreateDenom(percosis.TokenFactoryKeeper, percosis.BankKeeper, ctx, creator, &emptyDenom)
	require.NoError(t, err)
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// when
			gotErr := wasmbinding.PerformMint(percosis.TokenFactoryKeeper, percosis.BankKeeper, ctx, creator, spec.mint)
			// then
			if spec.expErr {
				require.Error(t, gotErr)
				return
...
func TestBurn(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			err := wasmbinding.PerformMint(percosis.TokenFactoryKeeper, percosis.BankKeeper, ctx, creator, mintBinding)
			require.NoError(t, err)
			...
			err = wasmbinding.PerformMint(percosis.TokenFactoryKeeper, percosis.BankKeeper, ctx, creator, emptyDenomMintBinding)
			require.NoError(t, err)

			// when
			gotErr := wasmbinding.PerformBurn(percosis.TokenFactoryKeeper, ctx, creator, spec.burn)
			// then
			if spec.expErr {
				require.Error(t, gotErr)
				return
