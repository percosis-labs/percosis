...
On Percosis mainnet, we only utilize one identifier, with a time interval of `one day`.
...
The Epochs module keeps a single [`EpochInfo`](https://github.com/percosis-labs/percosis/blob/b4befe4f3eb97ebb477323234b910c4afafab9b7/proto/percosis/epochs/genesis.proto#L12) per identifier.
...
```sh
percosisd query epochs epoch-infos
```
...
```sh
percosisd query epochs current-epoch [identifier]
```
...
```sh
percosisd query epochs current-epoch day
```
