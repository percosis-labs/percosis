...
This is a safety control, intended to protect assets on percosis in event of:

* a bug/hack on percosis
...
- [Dragonberry IBC bug](https://forum.cosmos.network/t/ibc-security-advisory-dragonberry/7702) (can't yet disclose amount at risk, but was saved due to being found first by altruistic Percosis core developers)

In the presence of a software bug on Percosis, IBC itself, or on a counterparty chain, we would like to prevent the bridge from being fully depegged.
...
* Per denomination rate limits
   - allows safety statements like "Only 30% of Stars on Percosis can flow out in one day" or "The amount of Atom on Percosis can at most double per day".
* Per channel rate limits
   - Limit the total inflow and outflow on a given IBC channel, based on "USDC" equivalent, using Percosis as the price oracle.
...
The "Outflow" side of a rate limit is protection against a bug on Percosis OR IBC.
...
So this parameterization has to contend with being a tradeoff of withdrawal liveness in high volatility periods vs being a crucial safety rail, in event of on-Percosis bug.
...
* Channel: The channel on the Percosis side: `packet.SourceChannel` for sends, and `packet.DestinationChannel` for receives. 
* Denom: The denom of the token being transferred as known on the Percosis side (more on that bellow)
* Channel Value: The total value of the chanel denominated in `Denom` (i.e.: channel-17 is worth 10k perco).  
...
#### Notes on Denom
We always use the the denom as represented on Percosis. For native assets that is the local denom, and for non-native 
...
This behaves slightly different if the asset is an percosis asset that was sent to the counterparty and is being
...
* For non-native tokens (`ibc/...`), the channel value should be the supply of those tokens in Percosis
* For native tokens, the channel value should be the total amount of tokens in escrow across all ibc channels

The later ensures the limits are lower and represent the amount of native tokens that exist outside Percosis. This is 
...
Percosis for all denoms (i.e.: treat native and non-native tokens the same way). Once that ticket is fixed, we will 
...
This means that if we have a daily quota of 1% of the perco supply, and the channel value is 1M perco at the beginning of 
the quota, no more than 100k perco can transferred during that day. If 10M perco were to be minted or IBC'd in during that
...
This integration can be seen in [percosis/app/keepers/keepers.go](https://github.com/percosis-labs/percosis/blob/main/app/keepers/keepers.go)
...
* Analyze changing denom-based rate limits, to just overall withdrawal amount for Percosis