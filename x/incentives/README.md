...
- For perpetual ones, it distribute all the tokens at a single time and somewhere else put the tokens regularly to distribute the tokens, it's mainly used to distribute minted PERCO tokens to LP token stakers.
...
Anyone can create a gauge and add rewards to the gauge. There is no way to withdraw gauge rewards other than distribution. Governance proposals can be raised to match the external incentive tokens with equivalent Perco incentives (see for example: [proposal 47](https://www.mintscan.io/percosis/proposals/47)).
...
- **`Perpetual gauges`** distribute all their tokens at a single time and only distribute their tokens again once the gauge is refilled (this is mainly used to distribute minted PERCO tokens to LP token stakers). Perpetual gauges persist and will re-disburse tokens when refilled (there is no "active" period)
...
```sh
percosisd tx incentives create-gauge [lockup_denom] [reward] [flags]
```
...
```bash
percosisd tx incentives create-gauge gamm/pool/3 10000ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4 \
--duration 24h  --start-time 1640081402 --epochs 2 --from WALLET_NAME --chain-id percosis-1
```
...
```bash
percosisd tx incentives create-gauge ibc/27394FB092D2ECCD56123C74F36E4C1F926001CEADA9CA97EA622B25F41E5EB2 \
1000000000ibc/46B44899322F3CD854D2D46DEEF881958467CDD4B3B10086DA49296BBED94BED --perpetual --duration 168h \
--from WALLET_NAME --chain-id percosis-1
```
...
```sh
percosisd tx incentives add-to-gauge [gauge_id] [rewards] [flags]
```
...
```bash
percosisd tx incentives add-to-gauge 1914 500000000ibc/46B44899322F3CD854D2D46DEEF881958467CDD4B3B10086DA49296BBED94BED \
--from WALLET_NAME --chain-id percosis-1
```
...
```sh
percosisd query incentives active-gauges [flags]
```

::: details Example

```bash
percosisd query incentives active-gauges
```
...
```sh
percosisd query incentives active-gauges-per-denom [denom] [flags]
```
...
```bash
percosisd query incentives active-gauges-per-denom gamm/pool/341
```
...
```sh
percosisd query incentives distributed-coins [flags]
```

::: details Example

```bash
percosisd query incentives distributed-coins
```
...
```sh
percosisd query incentives gauge-by-id [id] [flags]
```
...
```sh
percosisd query incentives gauge-by-id 1
```
...
```sh
percosisd query incentives gauges [flags]
```
...
```bash
percosisd query incentives gauges --limit 2000
```
...
```sh
percosisd query incentives to-distribute-coins [flags]
```

::: details Example

```bash
percosisd query incentives to-distribute-coins
```
...
```sh
percosisd query incentives upcoming-gauges [flags]
```

::: details Example

```bash
percosisd query incentives upcoming-gauges
```
