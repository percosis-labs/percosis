...
// For ex: delegate 10perco with validator-set {ValA -> 0.5, ValB -> 0.3, ValC -> 0.2}
// our delegate logic would attempt to delegate 5perco to A , 2perco to B, 3perco to C
// nolint: staticcheck
...
// undelegate 6perco with validator-set {ValA -> 0.5, ValB -> 0.3, ValC -> 0.2}
// our undelegate logic would attempt to undelegate 3perco from A, 1.8perco from B, 1.2perco from C
// nolint: staticcheck
...
// ForceUnlockBondedPerco allows breaking of a bonded lockup (by ID) of perco, of length <= 2 weeks.
// We want to later have perco incentives get auto-staked, we want people w/ no staking positions to
// get their perco auto-locked. This function takes all that perco and stakes according to your
...
func (k Keeper) ForceUnlockBondedPerco(ctx sdk.Context, lockID uint64, delegatorAddr string) (sdk.Coin, error) {
	lock, lockedPercoAmount, err := k.validateLockForForceUnlock(ctx, lockID, delegatorAddr)
	if err != nil {
		return sdk.Coin{}, err
	}
	...
	// Takes unlocked perco, and delegate according to valset pref
	unlockedPercoCoin := sdk.Coin{Denom: appParams.BaseCoinUnit, Amount: lockedPercoAmount}

	return unlockedPercoCoin, nil
}
...
func (k Keeper) validateLockForForceUnlock(ctx sdk.Context, lockID uint64, delegatorAddr string) (*lockuptypes.PeriodLock, sdk.Int, error) {
	...
	lockedPercoAmount := sdk.NewInt(0)
	...
	if coin.Denom == appParams.BaseCoinUnit {
		lockedPercoAmount = lockedPercoAmount.Add(coin.Amount)
	}

	// check if there is enough ufury token in the lock
	if lockedPercoAmount.LTE(sdk.NewInt(0)) {
		return nil, sdk.Int{}, fmt.Errorf("lock does not contain perco denom, or there isn't enough perco to unbond")
	}
	...
	return lock, lockedPercoAmount, nil
}
