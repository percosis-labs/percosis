...
func (s *KeeperTestSuite) TestUnDelegateFromValidatorSet() {
	...
	tests := []struct {
	...
	}{
		{
			...
			delegator:      sdk.AccAddress([]byte("addr1---------------")),
			coinToStake:    sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(20_000_000)), // delegate 20perco
			coinToUnStake:  sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(10_000_000)), // undelegate 10perco
			expectedShares: []sdk.Dec{sdk.NewDec(2_000_000), sdk.NewDec(3_300_000), sdk.NewDec(1_200_000), sdk.NewDec(3_500_000)},
			setValSet:      true,
			expectPass:     true,
		},
		{
			...
			delegator:      sdk.AccAddress([]byte("addr2---------------")),
			coinToStake:    sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(20_000_000)),                                           // delegate 20perco
			coinToUnStake:  sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(15_000_000)),                                           // undelegate 15perco
	...
	for _, test := range tests {
		s.Run(test.name, func() {
			s.FundAcc(test.delegator, amountToFund) // 100 perco
...
func (s *KeeperTestSuite) TestWithdrawDelegationRewards() {
	...
	tests := []struct {
	...
	}{
		{
			...
			delegator:           sdk.AccAddress([]byte("addr1---------------")),
			coinsToDelegate:     sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(20_000_000)), // delegate 20perco
...
func (s *KeeperTestSuite) TestDelegateBondedTokens() {
	...
	tests := []struct {
		...
		delegator            sdk.AccAddress
		lockId               uint64
		expectedUnlockedPerco sdk.Coin
		expectedDelegations  []sdk.Dec
		setValSet            bool
		expectPass           bool
	}{
		{
			name:                 "DelegateBondedTokens with existing perco denom lockId, bonded and <= 2 weeks bond duration",
			delegator:            sdk.AccAddress([]byte("addr1---------------")),
			lockId:               testLock[0].ID,
			expectedUnlockedPerco: sdk.NewCoin(appParams.BaseCoinUnit, sdk.NewInt(60_000_000)), // delegator has 100perco and creates 5 locks 10perco each, forceUnlock only 1 lock
	...
	for _, test := range tests {
		s.Run(test.name, func() {
			...
			if test.expectPass {
				...
				s.Require().Equal(test.expectedUnlockedPerco, balance)
