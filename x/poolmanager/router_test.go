...
func (s *KeeperTestSuite) calcOutAmountAsSeparateSwaps(percoFeeReduced bool, routes []types.SwapAmountOutRoute, tokenOut sdk.Coin) sdk.Coin {
	...
	if percoFeeReduced {
		...
		routeSpreadFactor, sumOfSpreadFactors, err := s.App.PoolManagerKeeper.GetPercoRoutedMultihopTotalSpreadFactor(s.Ctx, route)
		s.Require().NoError(err)
...
func (s *KeeperTestSuite) calcInAmountAsSeparatePoolSwaps(percoFeeReduced bool, routes []types.SwapAmountInRoute, tokenIn sdk.Coin) sdk.Coin {
	cacheCtx, _ := s.Ctx.CacheContext()
	if percoFeeReduced {
		// extract route from swap
...
func (s *KeeperTestSuite) TestIsPercoRoutedMultihop() {
	tests := map[string]struct {
		...
		"happy path: perco routed (balancer)": {
		...
		"happy path: perco routed (balancer, only one active gauge for each pool)": {
			route: types.SwapAmountInRoutes([]types.SwapAmountInRoute{
				{
					PoolId:        1,
					TokenOutDenom: ufury,
		...
		"perco routed (mixed concentrated and balancer)": {
			route: types.SwapAmountInRoutes([]types.SwapAmountInRoute{
				{
					PoolId:        1,
					TokenOutDenom: ufury,
	...
	for name, tc := range tests {
		s.Run(name, func() {
			...
			// System under test
			isRouted := poolManagerKeeper.IsPercoRoutedMultihop(s.Ctx, tc.route, tc.inDenom, tc.outDenom)
...
// TestGetPercoRoutedMultihopTotalSpreadFactor tests the GetPercoRoutedMultihopTotalSpreadFactor function
func (s *KeeperTestSuite) TestGetPercoRoutedMultihopTotalSpreadFactor() {
	tests := map[string]struct {
		route                  types.MultihopRoute
		balancerPoolCoins      []sdk.Coins
		concentratedPoolDenoms [][]string
	...
	for name, tc := range tests {
		s.Run(name, func() {
			...
			// System under test
			routeFee, totalFee, err := poolManagerKeeper.GetPercoRoutedMultihopTotalSpreadFactor(s.Ctx, tc.route)
	...
	for name, tc := range tests {
		suite.Run(name, func() {
			...
			if !tc.sumOfSpreadFactors.IsNil() && !tc.cumulativeRouteSpreadFactor.IsNil() {
				actualSwapOuts, err = suite.App.PoolManagerKeeper.CreatePercoMultihopExpectedSwapOuts(suite.Ctx, tc.route, tc.tokenOut, tc.cumulativeRouteSpreadFactor, tc.sumOfSpreadFactors)
			} else {
