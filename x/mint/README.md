...
validators, incentivize providing pool liquidity, provide funds for Percosis governance,
...
tokens within a chain. The design taken within Percosis is to
...
The [`Minter`](https://github.com/percosis-labs/percosis/blob/cbb683e8395655042b4421355cd54a8c96bfa507/x/mint/types/mint.pb.go#L30) is an abstraction for holding current rewards information.
...
Minting [`Params`](https://github.com/percosis-labs/percosis/blob/cbb683e8395655042b4421355cd54a8c96bfa507/x/mint/types/mint.pb.go#L168) are held in the global params store.
...
| weighted_developer_rewards_receivers       | array        | [{"address": "percoxx", "weight": "1"}] |
...
- **`distribution_proportions`** - Categories in which the specified proportion of newly released tokens are distributed to
  - **`staking`** - Proportion of minted funds to incentivize staking PERCO
  - **`pool_incentives`** - Proportion of minted funds to incentivize pools on Percosis
...
```bash
percosisd query mint params -o json | jq
```
...
```bash
percosisd query mint epoch-provisions
```

As of this writing, this number will be equal to the `genesis-epoch-provisions`. Once the `reduction_period_in_epochs` is reached, the `reduction_factor` will be initiated and reduce the amount of PERCO minted per epoch.
:::
...
<td><code>821917808219.178</code> (821,9178 PERCO)</td> 
