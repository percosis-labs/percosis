...
// func GetPercoSnapshot(inputFile string) (Snapshot, error) {
...
// func CosmosToPercoAddress(cosmosAddr string) (string, error) {
// 	_, bz, err := bech32.DecodeAndConvert(cosmosAddr)
// 	if err != nil {
// 		return "", err
// 	}

// 	convertedAddr, err := bech32.ConvertAndEncode("perco", bz)
// 	if err != nil {
// 		panic(err)
// 	}
...
// Both PERCO and ION recipients. If erroring, ensure to 'git lfs pull'

// Example:
// 	percosisd import-genesis-accounts-from-snapshot networks/cosmoshub-3/snapshot.json networks/percosis-1/ions.json output_address.json
// `,
...
// 			// Read snapshot file
// 			snapshot, err := GetPercoSnapshot(args[0])
// 			if err != nil {
// 				return err
// 			}
...
// 				if !acc.PercoBalance.Equal(sdk.ZeroInt()) {
// 					percoAddr, err := CosmosToPercoAddress(acc.AtomAddress)
// 					if err != nil {
// 						return err
// 					}

// 					airdropAddrs[percoAddr] = true
// 				}
// 			}

// 			for addr := range ionAmts {
// 				ionAddr, err := CosmosToPercoAddress(addr)
// 				if err != nil {
// 					return err
// 				}
...
// Example:
// 	percosisd import-genesis-accounts-from-snapshot ../snapshot.json ../ions.json
// 	- Check input genesis:
...
// 				address, err := CosmosToPercoAddress(addr)
// 				if err != nil {
// 					return err
// 				}
...
// 			normalizationFactor := genesisParams.AirdropSupply.ToDec().QuoInt(snapshot.TotalPercosAirdropAmount)
...
// 			for _, acc := range snapshot.Accounts {
// 				// convert cosmos address to perco address
// 				address, err := CosmosToPercoAddress(acc.AtomAddress)
// 				if err != nil {
// 					return err
// 				}

// 				// skip accounts with 0 balance
// 				if !acc.PercoBalanceBase.IsPositive() {
// 					continue
// 				}

// 				// get normalized perco balance for account
// 				normalizedPercoBalance := acc.PercoBalance.ToDec().Mul(normalizationFactor)

// 				// initial liquid amounts
// 				// We consistently round down to the nearest ufury
// 				liquidAmount := normalizedPercoBalance.Mul(sdk.MustNewDecFromStr("0.2")).TruncateInt() // 20% of airdrop amount
...
// 				// claimable balances
// 				claimableAmount := normalizedPercoBalance.Mul(sdk.MustNewDecFromStr("0.8")).TruncateInt()
...
// 			// The total airdrop perco is a smidge short (~1 perco) short of the stated 50M supply.
// 			// This is due to consistently rounding down.
// 			// We place this remaining 1 perco into the community pool at genesis
