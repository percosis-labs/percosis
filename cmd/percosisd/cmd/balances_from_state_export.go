...
Example:
	percosisd export-derive-balances ../genesis.json ../snapshot.json
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, unbonding := range stakingGenesis.UnbondingDelegations {
				...
				unbondingPercos := sdk.NewInt(0)
				for _, entry := range unbonding.Entries {
					unbondingPercos = unbondingPercos.Add(entry.Balance)
				}

				acc.UnbondingStake = acc.UnbondingStake.Add(unbondingPercos)
			...
			for _, delegation := range stakingGenesis.Delegations {
				...
				stakedPercos := delegation.Shares.MulInt(val.Tokens).Quo(val.DelegatorShares).RoundInt()

				acc.Staked = acc.Staked.Add(stakedPercos)
...
Example:
	percosisd staked-to-csv ../balances.json ../airdrop.csv
`,
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx := client.GetClientContextFromCmd(cmd)
