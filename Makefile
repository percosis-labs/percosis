...
build-reproducible-amd64: go.sum
	...
	$(DOCKER) buildx build \
		...
		-t percolabs:local-amd64 \
		--load \
		-f Dockerfile .
	$(DOCKER) rm -f percobinary || true
	$(DOCKER) create -ti --name percobinary percolabs:local-amd64
...
build-reproducible-arm64: go.sum
	...
	$(DOCKER) buildx build \
		...
		-t percolabs:local-arm64 \
		--load \
		-f Dockerfile .
	$(DOCKER) rm -f percobinary || true
	$(DOCKER) create -ti --name percobinary fanfury:local-arm64
...
docker-build-debug:
	...
	@DOCKER_BUILDKIT=1 docker tag percolabs:${COMMIT} percolabs:debug
